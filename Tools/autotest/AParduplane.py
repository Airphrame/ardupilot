...
def fly_mission_with_rtl(mavproxy, mav, filename, height_accuracy=-1, target_altitude=None):
    '''fly auto takeoff mission and initiate an RTL'''
    # wait for EKF to settle
    wait_seconds(mav, 15)
    print("Flying mission %s" % filename)
    mavproxy.send('wp load %s\n' % filename)
    mavproxy.expect('Flight plan received', timeout=60)
    ...
    # a bit faster again, straighten rudder
    mav.recv_match(condition='VFR_HUD.groundspeed>12', blocking=True)
    if not wait_waypoint(mav, 1, 3, max_dist=60):
        return False
    print("Flying home in RTL")
    mavproxy.send('mode RTL\n')
    wait_mode(mav, 'RTL')
    if not wait_location(mav, homeloc, accuracy=120,
                        target_altitude=homeloc.alt+100, height_accuracy=20,
                        timeout=180):
        return False
    print("fly mission with rtl completed")
    return True

def fly_GUIDED(mavproxy, mav, filename):
    global homeloc
    ''' fly auto then to a guided location '''
    print("Flying mission %s" % filename)
    mavproxy.send('wp load %s\n' % filename)
    ...
    wait_mode(mav, 'AUTO')
    if not wait_waypoint(mav, 1, 4, max_dist=60):
        return False
    mavproxy.send('mode guided\n')
    mavproxy.expect('GUIDED')
    wait_mode(mav, 'GUIDED')
    mavproxy.send('guided %f %f 500 \n' % (homeloc.lat, homeloc.lng))
    mavproxy.expect('GUIDED')
    if not wait_location(mav, homeloc, accuracy=120,
                        target_altitude=homeloc.alt+500, height_accuracy=20,
                        timeout=180):
        return False
    print("fly guided completed")
    return True
...
def fly_mission(mavproxy, mav, filename, height_accuracy=-1, target_altitude=None):
    ...
    mavproxy.send('mode auto\n') # auto mode
    wait_mode(mav, 'AUTO')
    if not wait_waypoint(mav, 1, 16, max_dist=60):
        return False
    if not wait_groundspeed(mav, 0, 3, timeout=60):
        return False
    print("fly mission completed")
    return True
...
def fly_ArduPlane(viewerip=None, map=False):
    ...
    mavproxy.send("param load %s/APArduPlane.parm\n" % testdir)
    ...
    # things to test for: auto takeoff. auto mission. RTL to land. bidir-landing 
    ...
    try:
        ...
        if not fly_mission_with_rtl(mavproxy, mav, os.path.join(testdir, "test_mission.txt"), height_accuracy = 10):
            print("Failed mission with RTL")
            failed = True
        if not fly_GUIDED(mavproxy, mav, os.path.join(testdir, "test_mission.txt")):
            print("Failed GUIDED")
            failed = True
        if not fly_mission(mavproxy, mav, os.path.join(testdir, "test_mission.txt"), height_accuracy = 10,
